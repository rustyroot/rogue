...
(** [elephant], on doit se souvenir de sa position (hérité depuis [entity]) de
    son état et durée (state et stunned (resp. charge)) *)
...
(** [camel_on_sight_direction current_position direction] renvoit [true] si le
    chameau est visible dans la [direction], [false] sinon *)
let rec camel_on_sight_direction (current_position : int * int)
...
(** [camel_on_sight] regarde dans les 4 directions depuis [current_position] si
    le chameau y est en vue. Si c'est le cas, on renvoie la direction du chameau
    sinon (0,0) *)
let camel_on_sight (current_position : int * int) : int * int =
  if camel_on_sight_direction (current_position ++ (-1, 0)) (-1, 0) then (-1, 0)
  else if camel_on_sight_direction (current_position ++ (1, 0)) (1, 0) then
    (1, 0)
  else if camel_on_sight_direction (current_position ++ (0, 1)) (0, 1) then
    (0, 1)
  else if camel_on_sight_direction (current_position ++ (0, -1)) (0, -1) then
    (0, -1)
  else (0, 0)

(** [elephant elephant_instance] calcule le nouvel état de l'éléphant en
    fonction du précédent
    - Si l'éléphant était en [Looking] du chameau et qu'il l'a trouvé, il passe
      en [Charging]
    - Si il était en [Charging] ou [Stunned], il y reste jusqu'à la fin du
      compteur puis il repasse en [Looking]
...
let rec elephant (elephant_instance : elephant) : unit =
  ...
  let new_position =
    ...
    | Charging ->
        ...
        begin if elephant_instance#get_charge = 0 then
    ...
    | Stunned ->
        ...
        if elephant_instance#get_stunned = 0 then begin
          elephant_instance#set_state Looking
        end;
        current_position
  in
