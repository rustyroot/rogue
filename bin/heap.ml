...
(** [init capacity compare first_elt] crée un tas min de capacité [capacity]
    où les élements sont ordonnés selon la relation binaire [compare].*)
let init (capacity : int) (compare : 'a -> 'a -> bool) (first_elt : 'a) : 'a heap =
  {capacity = capacity; 
   index = 1;
   array = Array.make capacity first_elt;
   gt = compare}

(** [is_empty heap] renvoit [true] si et seulement si le tas ne contient aucun élément.*)
let is_empty (heap : 'a heap) : bool =
  (heap.index = 0)

(** [has_a_right_child heap i] renvoit [true] si et seulement si le noeud [i] a un fils droit.*)
let has_a_right_child (heap: 'a heap) (i : int) =
  2*i+1 < heap.index

(** [has_a_left_child heap i] renvoit [true] si et seulement si le noeud [i] a un fils gauche.*)
let has_a_left_child (heap: 'a heap) (i : int) =
  2*i+2 < heap.index

(** [is_greater_than_his_right_child heap i] renvoit [true] si et 
    seulement si le noeud [i] a un fils droit plus petit que lui. *)
let is_greater_than_his_right_child (heap : 'a heap) (i : int) =
  has_a_left_child heap i && heap.gt (heap.array.(i)) (heap.array.(2*i+1))

(** [is_greater_than_his_left_child heap i] renvoit [true] si et 
    seulement si le noeud [i] a un fils gauche plus petit que lui. *)
let is_greater_than_his_left_child (heap : 'a heap) (i : int) =
  has_a_right_child heap i &&  heap.gt (heap.array.(i)) (heap.array.(2*i+2))

(** [is_greater_than_his_childs heap i] renvoit [true] si et 
    seulement si le noeud [i] a un fils plus petit que lui. *)
let is_greater_than_his_childs (heap: 'a heap) (i : int) =
  is_greater_than_his_left_child heap i || is_greater_than_his_right_child heap i
...
let pop (heap : 'a heap) : 'a  =
  if is_empty heap then 
    raise Pop_in_empty_heap;
...
let mem (heap : 'a heap) (elt : 'a) : bool =
  let rec parcourir (i : int) =
    if i > heap.index then
      false
    else (
      if heap.array.(i) = elt then
        true
      else
        parcourir (i+1)
    )
