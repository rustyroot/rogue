...
(** L'effet [End_of_level] indique que le chameau a trouvé la clé et qu'il faut
    changer de level*)
type _ Effect.t += End_of_level : unit t

(** L'effet [End_of_game] indique que le chameau a été tué et qu'il faut
    terminer le jeu *)
...
let move (old_position : int * int) (new_position : int * int) : int * int =
  ...
  | Camel ->
      if get old_position != Camel then (
        (* Le chameau ne se retire pas de vie quand il se rencontre lui même *)
        lives := !lives - 1;
...
(** [random_direction ()] Choisie alléatoirement une direction et renvoie le
    changement à appliquer sur les coordonnées du serpent pour aller dans la
...
(** [get_free_nearby_cell position] renvoit la liste des cases [Empty] parmi le
    carré de 3x3 cases autour de position. *)
let get_free_nearby_cell (position : int * int) : (int * int) list =
  let x, y = position in
  let rec check_nearby_cell (i : int) (j : int)
      (free_nearby_cell : (int * int) list) : (int * int) list =
    if i <= 1 then
      match get (x + i, y + j) with
      | Empty -> check_nearby_cell (i + 1) j ((x + i, y + j) :: free_nearby_cell)
      | _ -> check_nearby_cell (i + 1) j free_nearby_cell
    else if j <= 1 then check_nearby_cell (-1) (j + 1) free_nearby_cell
    else free_nearby_cell
  in
  check_nearby_cell (-1) (-1) []

exception No_cell_avaible

(** [get_random_nearby_cell position] choisit aléatoirement une cases [Empty]
    parmi le carré de 3x3 cases autour de position. Si aucune n'est libre alors
    [No_cell_avaible] est soulevée. *)
let get_random_nearby_cell (position : int * int) : int * int =
  ...
  if nb_free_nearby_cell = 0 then raise No_cell_avaible
  else List.nth free_nearby_cell (Random.int nb_free_nearby_cell)
