...
class egg entity_instance =
  object
    ...
    method set_generation_remaining new_generation_remaining =
      generation_remaining <- new_generation_remaining

    method get_nb_turns_before_generation = nb_turns_before_generation

    method set_nb_turns_before_generation new_nb_turns_before_generation =
      nb_turns_before_generation <- new_nb_turns_before_generation
  end

(** [spider spider_instance] effectue tous les prochains tours de l'araigné à
    partir des paramètres [spider_instance] (choisir aléatoirement une entrée,
    se déplacer en conséquence, essayer de créer un oeuf, recommencer) *)
...
(** [spawn_egg spider_position] tente de créer un oeuf sur une case aléatoire
    autour de [spider_position], elle peut échouer en renvoyant
    [No_cell_avaible]*)
...
(** [egg egg_instance] effectue tous les prochains tours de l'oeuf à partir des
    paramètres [egg_instance] (attendre, créer un araignée, cela 3 fois avant de
    terminer)*)
and egg (egg_instance : egg) : unit =
  let nb_turns_before_generation =
    egg_instance#get_nb_turns_before_generation
  in
  if nb_turns_before_generation > 0 then begin
    egg_instance#set_nb_turns_before_generation (nb_turns_before_generation - 1)
  end
  else begin
    let egg_position = egg_instance#get_pos in
    begin try spawn_spider egg_position with No_cell_avaible -> ()
    end;
    egg_instance#set_nb_turns_before_generation 20;
    egg_instance#set_generation_remaining
      (egg_instance#get_generation_remaining - 1)
  end;

  if egg_instance#get_generation_remaining > 0 then begin
    perform End_of_turn;
    egg egg_instance
  end
  else set egg_instance#get_pos Empty

(** [spawn_spider egg_position] tente de créer une araigné sur une case
    aléatoire autour de [egg_position], elle peut échouer en renvoyant
    [No_cell_avaible]*)
