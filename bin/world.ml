...
let rec safe_random_position () : int * int =
  ...
  if get position = Empty then position else safe_random_position ()
...
let fill_world () : unit =
  ...
  for _ = 0 to nb_cactus do
    set (random_position ()) Cactus
  done

(** [override_world] efface le monde actuel et le remplace avec [w] A n'utiliser
    que dans test_rogue.ml pour override [world]*)
let override_world (w : cell array array) : unit =
  Array.iteri (fun i line -> Array.iteri (fun j cell -> set (i, j) cell) line) w
