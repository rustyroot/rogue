...
(** [affine_function dst src] renvoit l'unique fonction affine passant par [src]
    et [dst].*)
...
(** [descrete_ray_cast src dst] renvoit la liste des cases du plateau
    intersectées par le segment allant src à dst. À noté que la liste renvoyée
...
let rec descrete_ray_cast (src : int * int) (dst : int * int) : (int * int) list
  ...
  if src = dst then [ src ]
    ...
    if x' < x then
      ...
      in
      let rec construct_list (x : int) (left_y : int) (acc : (int * int) list) :
          (int * int) list =
        if x > x' then acc
        else
      ...
      let rec cut_out_of_range (cell_list : (int * int) list)
        ...
        | head :: tail when begin_pass ->
            head :: cut_out_of_range tail begin_pass
...
(** [enlight path_of_light] copie les éléments de [world] dans [shadowed_world]
    si ceux-ci sont touchés par le rayon de lumière discret. *)
...
(** [enlighten_the_world camel_position] met à jour [shadowed_world] en tirant
    des rayons lumières discretes depuis le chameau vers l'ensemble des cases du
    plateau. *)
